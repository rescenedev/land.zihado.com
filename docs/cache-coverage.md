...
- ⚠️ **빈 구 aptmap(거래 0건, 예: 41190/41590 경기)**: `aggs.length>0`일 때만 캐시 → 빈 구는 KV 캐시 안 됨, colo별 첫조회 빈 집계 스캔 ~1300ms(이후 edge 캐시). 사용자 거의 안 감. TODO: 빈 결과도 캐시(`aggs.length>=0`)로 다음 배포 때 수정.
- 💡 **CDN 오염 시 대응**: long-tail 을 프록시로 대량 워밍하면 Vercel CDN 에 쌓여 코어 evict. `vercel cache purge --type cdn -y` 로 제거 후 코어만 프록시 재워밍(워커 KV 는 무영향). 실측: 퍼지 후 당월 코어 무재워밍 자가유지 확인.
