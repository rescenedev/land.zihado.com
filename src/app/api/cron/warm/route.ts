...
export async function GET(req: Request): Promise<Response> {
  ...
  // ── 5) 적재 manifest 워밍: 프록시(/api/[...path])가 미적재 구를 서울에서 즉시 filling 판별할 때
  //       읽는 /api/ingested 를 서울엣지에 적재(프록시가 읽는 URL 과 동일: months=13). 프로비저닝 불필요.
  stat.manifest = await warm([`${ORIGIN}/api/ingested?months=13`]);
