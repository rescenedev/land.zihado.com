...
export default function HomeClient({
  ...
  // 사이드바에서 현재 라우트를 다시 클릭(예: 대시보드) → 드릴다운/오버레이 초기화 = 전국 복귀
  useEffect(() => {
    const reset = () => {
  ...
  return (
    <>
      <div className="mx-auto w-[92%] max-w-[1800px] px-2 py-7">
        ...
        {/* 시도 필터 칩 (월=상단 기준월, 뷰=좌측 사이드바로 이관 → 여기는 칩만) */}
        <div className="mb-5 flex flex-wrap content-start gap-1.5">
          {SIDO_TABS.map((sd) => {
            const active =
              sd === "전국"
        ...
        <footer className="mt-12 space-y-1.5 border-t border-slate-800/60 pt-4 text-center text-[11px] text-slate-600">
          <div className="text-slate-500">
            ...
            {" · "}엣지 HIT p50 ~10ms{" · "}p95 ~12ms{" · "}p99 ~20ms{" · "}콜드 revalidation ~50ms{" · "}엣지캐시 HIT
            <span className="ml-1 text-slate-600">(oha keep-alive 실측 10k req · icn1 컴퓨트 · 한국↔서울엣지 왕복)</span>
          </div>
