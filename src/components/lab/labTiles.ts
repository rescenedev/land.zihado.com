...
export type LabTile = {
  ...
  href?: string; // 기존 라우트 매핑(있으면 바로 이동), 없으면 준비중
  related?: { label: string; href: string }[]; // 준비중 화면에서 안내할 관련 메뉴
};

// 참고 이미지("부동산 스터디") 순서 유지.
export const LAB_TILES: LabTile[] = [
  { slug: "decline", label: "최근하락", desc: "직전 거래 대비 가격이 가장 많이 내린 단지", icon: "decline", color: "#3b82f6", href: "/today" },
  { slug: "top", label: "최고가", desc: "기간 내 최고가 실거래", icon: "top", color: "#f59e0b", href: "/today" },
  { slug: "rise", label: "최고상승", desc: "직전 거래 대비 상승폭이 가장 큰 단지", icon: "rise", color: "#f43f5e", href: "/today" },
  ...
  { slug: "hot-complex", label: "많이산단지", desc: "거래가 가장 활발한 단지 랭킹", icon: "hotComplex", color: "#0ea5e9", related: [{ label: "오늘의 실거래", href: "/today" }] },
...
// 준비중(href 없는) 타일만 정적 생성 대상.
export const LAB_SOON_SLUGS: string[] = LAB_TILES.filter((t) => !t.href).map((t) => t.slug);
