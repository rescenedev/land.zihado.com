...
app.get("/api/recent", async (c) => {
  ...
  const payload = await cachedJson(c.env, recentKey, recentTtl, async () => {
    ...
    // 결과가 비면(오늘=신고지연) 월·스코프 최신 계약일 + 그 날의 건수를 latest/latestCount 로 —
    // SSR 이 같은 응답에서 점프 타깃과 "최신일 N건" 안내를 얻어 별도 프로브(직렬 워커 왕복)를 안 탄다.
    let latest = enriched[0]?.dealDate ?? "";
    let latestCount = enriched.length;
    if (!latest) {
      const l = await latestDealDateInMonth(c.env, dataset, yyyymm, codes);
      if (l) { latest = l.date; latestCount = l.count; }
    }
    return { dataset, yyyymm, scope, date: exactDate ?? null, latest, latestCount, count: enriched.length, deals: enriched };
  });
